    ...
    <body class="background">
        <div class="col-md-6 col-lg-6 col-sm-8 col-xs-11 navbar-fixed-bottom rcorners text-center" style="margin-bottom: 5%;margin-left:10px;margin-right:10px;margin-top: 10%">
            <div id="pregunta1" name="pregunta1">
                <form action="../controller/preguntas9.php" method="post" id="myForm">
                        <?php
                $aux = 0;

                $sql = "SELECT * FROM preguntas INNER JOIN respuesta ON preguntas.id_pregunta=respuesta.id_pregunta WHERE preguntas.id_pregunta='124'";
                ...
                <a href="../view/encuestasContestar12.php" class="volver">&larr; Volver</a>
                
                </form>
                <script>
                    var a = document.getElementById('respuesta3').getElementsByTagName('li');
                    for (var i = 0; i < a.length; i++)
                    {
                        a[i].onclick = function ()
                        {          
                                // PRIMERO ELIMINO EL "<h2>" DEL PRINCIPIO Y LO REEMPLAZO POR VACIO Y LO GUARDO EN LA VARIABLE "r1"
                            var r1= this.innerHTML.replace("<h5>", "");
                            // LUEGO ELIMINO EL "</h2>" DEL FINAL Y LO REEMPLAZO POR VACIO Y LO GUARDO EN LA VARIABLE "r2"
