        ...
        <div class="col-md-6 col-lg-6 col-sm-8 col-xs-11 navbar-fixed-bottom rcorners text-center" style="margin-bottom: 5%;margin-left:10px;margin-right:10px;margin-top: 10%">
            <div id="pregunta1" name="pregunta1">
                <form action="../controller/preguntas2.php" method="post">
                        <?php
                $aux = 0;
                $sql = "SELECT * FROM preguntas INNER JOIN respuesta ON preguntas.id_pregunta=respuesta.id_pregunta WHERE preguntas.id_pregunta='116'";
                $tabla2 = getResultSQL($sql);
                while ($row = $tabla2->fetch_array(MYSQLI_BOTH)) {
                    if ($aux == 0) { ?>
                        <h1 id="pregunta1" name="pregunta1" class"blanco" style="color:white"><?php echo $row['pregunta']." mes/año"; ?></h1>
                ...
                ?>
                        ...
                        <h2><input type="date" id="respuesta_mes" name="respuesta_mes"></h2>
                <input type="submit" name="btnaceptar1" value="Aceptar">
                <a href="../view/encuestasContestar4.php" class="volver">&larr; Volver</a>
